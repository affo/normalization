...
mpi_env_t get_mpi_env(int numtasks, int root, int rank, int length){
	openlog("NORM - ENV", LOG_CONS | LOG_PID | LOG_NDELAY, LOG_LOCAL1);
	...
	syslog(LOG_NOTICE, "Process %d --> chunk: %d\n", rank, chunk);
	return *env;
}

void normalize(int old_min, int old_max, int new_min, int new_max, mpi_env_t env){
	openlog("NORM - NORM", LOG_CONS | LOG_PID | LOG_NDELAY, LOG_LOCAL1);
	...
	syslog(LOG_NOTICE, "Process %d --> using %d threads\n", env.rank, omp_get_max_threads());
...
int get_max(mpi_env_t env){
	openlog("NORM - MAX", LOG_CONS | LOG_PID | LOG_NDELAY, LOG_LOCAL1);
	...
	#pragma omp parallel shared(receive_buffer, local_max)
	{
		...
		syslog(LOG_NOTICE, "Process %d --> Thread %d --> submax: %d\n", env.rank, omp_get_thread_num(), submax);

		#pragma omp critical
			if(submax > local_max) local_max = submax;
	}

	syslog(LOG_NOTICE, "Process %d --> local_max: %d\n", env.rank, local_max);

	MPI_Allreduce(&local_max, &global_max, 1, MPI_INT, MPI_MAX, MPI_COMM_WORLD);

	syslog(LOG_NOTICE, "Process %d --> global_max: %d\n", env.rank, global_max);
	return global_max;
}

int get_min(mpi_env_t env){
	openlog("NORM - MIN", LOG_CONS | LOG_PID | LOG_NDELAY, LOG_LOCAL1);
	...
	#pragma omp parallel shared(receive_buffer, local_min)
	{
		...
		syslog(LOG_NOTICE, "Process %d --> Thread %d --> submin: %d\n", env.rank, omp_get_thread_num(), submin);

		#pragma omp critical
			if(submin < local_min) local_min = submin;
	}

	syslog(LOG_NOTICE, "Process %d --> local_min: %d\n", env.rank, local_min);

	MPI_Allreduce(&local_min, &global_min, 1, MPI_INT, MPI_MIN, MPI_COMM_WORLD);

	syslog(LOG_NOTICE, "Process %d --> global_min: %d\n", env.rank, global_min);
	return global_min;
}
